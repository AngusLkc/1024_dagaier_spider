...
class ThreadManager(object):
    ...
    def __start__(self):
        for i in range(self.thread_num):
            i=ThreadWork(self.queue,self.shutdown,i) #i:值传递
            i.start()
    ...
    def run(self):
        while True:
            if self.shutdown.isSet():
                logging.info("线程ID：%s，检测到线程退出标志！"%(self.getName()))
                break
            try:
...
def dagaier(topicurl,title):
    ...
    if topic_req.status_code!=200:
        logging.warning("线程ID：%s，下载主题页失败, Status:%s, URL:%s"%(threading.currentThread().getName(),topic_req.status_code,topicurl))
        return False
    ...
    for item in imglist:
        if item.attr('src') is not None:
            ...
            logging.warning("线程ID：%s，读取帖子内图片URL失败, URL:%s"%(threading.currentThread().getName(),topicurl))
            return False
...
def downimg(url,title):
    ...
    if img_req.status_code==200:
        ...
        logging.warning("线程ID:%s，下载图片失败, Status:%s, URL:%s"%(threading.currentThread().getName(),img_req.status_code,url))
        return False

if __name__=='__main__':
    os.chdir(os.path.dirname(os.path.realpath(inspect.getfile(inspect.currentframe()))))
    work_manager=ThreadManager(10) #线程数
    ...
    offset=0
    error_count=0
    while offset<1: #主题列表页数
        offset+=1
        if error_count>=3:
            logging.error("遍历主题列表页失败！页码:%i"%(offset))
            error_count=0
            continue
        ...
        if Page_Obj.status_code!=200:
            error_count+=1
            logging.warn("下载主题列表分页失败：%i，重试:%i"%(offset,error_count))
            offset-=1
            continue
        error_count=0
